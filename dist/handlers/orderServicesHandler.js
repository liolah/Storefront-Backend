...
const userOrders = async (req, res) => {
    const orders = await ordersServices_1.default.getOrderByUserId(req.body.userId);
    res.json(orders);
};
exports.userOrders = userOrders;
const completedOrders = async (req, res) => {
    const orders = await ordersServices_1.default.completedOrders(req.body.userId);
    res.json(orders);
};
